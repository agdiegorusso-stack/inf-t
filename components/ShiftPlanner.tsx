...
import { UNASSIGNED_STAFF_ID, LONG_SHIFTS } from '../constants';
    ...
    const handleGenerate = useCallback(() => {
        ...
        setTimeout(() => {
            try {
                ...
                const staffStats: Record<string, { totalShifts: number; nightShifts: number; longShifts: number; weekendShifts: number }> = {};
                staffList.forEach(s => staffStats[s.id] = { totalShifts: 0, nightShifts: 0, longShifts: 0, weekendShifts: 0 });
                ...
                const assignShift = (staffId: string, dateStr: string, shiftCode: string) => {
                    ...
                    const dow = d.getDay();
                    if (dow === 0 || dow === 6) {
                        staffStats[staffId].weekendShifts++;
                    }
                };
    
                // Advanced eligibility and scoring helpers
                const MAX_CONSECUTIVE_WORK_DAYS = 6;
                const LONG_SHIFT_MONTHLY_CAP = 6;
    
                const isWorkingShiftCode = (code: string | undefined): boolean => {
                    if (!code) return false;
                    const def = getShiftDefinitionByCode(code);
                    if (!def) return false;
                    return def.time !== ShiftTime.Rest && def.time !== ShiftTime.Absence && def.time !== ShiftTime.OffShift;
                };
    
                const previousDateStr = (dateStr: string): string => {
                    const d = new Date(dateStr + 'T12:00:00');
                    d.setDate(d.getDate() - 1);
                    return formatDate(d);
                };
    
                const getConsecutiveWorkingDaysUpTo = (staffId: string, dateStr: string): number => {
                    let count = 0;
                    let curr = previousDateStr(dateStr);
                    while (staffAssignments[staffId]?.[curr] && isWorkingShiftCode(staffAssignments[staffId][curr])) {
                        count++;
                        curr = previousDateStr(curr);
                    }
                    return count;
                };
    
                const isEligibleCandidate = (s: Staff, shiftCode: string, dateStr: string): boolean => {
                    if (staffAssignments[s.id]?.[dateStr]) return false;
                    if (!isShiftAllowed(shiftCode, s, shiftDefinitions, teams)) return false;
    
                    const def = getShiftDefinitionByCode(shiftCode);
                    if (!def) return true;
    
                    const prevDate = previousDateStr(dateStr);
                    const prevShift = staffAssignments[s.id]?.[prevDate];
                    if (prevShift) {
                        const prevDef = getShiftDefinitionByCode(prevShift);
                        if (prevDef?.time === ShiftTime.Night) {
                            if (def.time === ShiftTime.Morning || def.time === ShiftTime.Afternoon || def.time === ShiftTime.FullDay) {
                                return false;
                            }
                        }
                        if (LONG_SHIFTS.includes(prevShift) && def.time === ShiftTime.Morning) {
                            return false;
                        }
                    }
    
                    const cons = getConsecutiveWorkingDaysUpTo(s.id, dateStr);
                    if (cons >= MAX_CONSECUTIVE_WORK_DAYS) return false;
    
                    if (LONG_SHIFTS.includes(shiftCode) && staffStats[s.id].longShifts >= LONG_SHIFT_MONTHLY_CAP) return false;
    
                    return true;
                };
    
                const candidateScore = (s: Staff, shiftCode: string, dateStr: string): number => {
                    let score = staffStats[s.id].totalShifts;
                    const def = getShiftDefinitionByCode(shiftCode);
                    if (def?.time === ShiftTime.Night) {
                        score += staffStats[s.id].nightShifts * 0.75;
                    }
                    const d = new Date(dateStr + 'T12:00:00');
                    const dow = d.getDay();
                    if (dow === 0 || dow === 6) {
                        score += staffStats[s.id].weekendShifts * 0.5;
                    }
                    const prev = previousDateStr(dateStr);
                    const prevShift = staffAssignments[s.id]?.[prev];
                    ...
                    for (const shiftCode of shiftCodesToFill) {
                        ...
                        if (assignedCount < minRequired) {
                            const candidates = staffList
                                .filter(s => isEligibleCandidate(s, shiftCode, dateStr))
                                .sort((a, b) => candidateScore(a, shiftCode, dateStr) - candidateScore(b, shiftCode, dateStr));
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    for (const shiftCode of shiftCodesToFill) {
                        ...
                        if (assignedCount < maxRequired) {
                            const candidates = staffList
                                .filter(s => isEligibleCandidate(s, shiftCode, dateStr))
                                .sort((a, b) => candidateScore(a, shiftCode, dateStr) - candidateScore(b, shiftCode, dateStr)); // Balance load including nights/weekends
                ...
                const longShiftAssignments: Record<string, number> = {}; // Track long shifts per staff per month
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    for (const shift of relevantShifts) {
                        ...
                        if (assignedCount < requiredCount) {
                            ...
                            // If this is a long shift, try to assign it to available staff within their limits
                            if (LONG_SHIFTS.includes(shiftCode)) {
                                log.push(`🔄 Tentativo di assegnare turni lunghi scoperti...`);
                                
                                // Get staff who are available for long shifts and within their monthly limit
                                const eligibleStaff = staffList.filter(staff => {
                                    // Check if staff is available for long shifts
                                    if (!staff.availableForLongShifts) return false;
                                    
                                    // Check if staff is eligible for this shift
                                    if (!isEligibleCandidate(staff, shiftCode, dateStr)) return false;
                                    
                                    // Check if staff is within their monthly long shift limit
                                    const [yearStr, monthStr] = dateStr.split('-');
                                    const staffLongShiftsThisMonth = Object.values(staffAssignments[staff.id] || {})
                                        .filter((date, shift) => {
                                            const [shiftYear, shiftMonth] = date.split('-');
                                            return shiftYear === yearStr && shiftMonth === monthStr && LONG_SHIFTS.includes(shift);
                                        }).length;
                                    
                                    const maxLongShifts = staff.maxLongShiftsPerMonth ?? 1;
                                    return staffLongShiftsThisMonth < maxLongShifts;
                                }).sort((a, b) => candidateScore(a, shiftCode, dateStr) - candidateScore(b, shiftCode, dateStr));
                                
                                // Assign long shifts to eligible staff
                                let assignedLongShifts = 0;
                                for (const staff of eligibleStaff) {
                                    if (assignedLongShifts >= deficit) break;
                                    
                                    assignShift(staff.id, dateStr, shiftCode);
                                    assignedLongShifts++;
                                    
                                    log.push(`✅ Assegnato turno lungo ${shiftCode} a ${staff.name} per coprire scoperto`);
                                }
                                
                                // Create uncovered shifts only for the remaining deficit
                                const remainingDeficit = deficit - assignedLongShifts;
                                for (let i = 0; i < remainingDeficit; i++) {
                                    finalUncoveredShifts.push({ 
                                        id: `uncovered-${dateStr}-${shiftCode}-${i}`, 
                                        date: dateStr, 
                                        staffId: UNASSIGNED_STAFF_ID, 
                                        shiftCode,
                                        // Add a special flag for dangerous long shifts
                                        ...(LONG_SHIFTS.includes(shiftCode) && { dangerousLongShift: true })
                                    });
                                }
                            } else {
                                // For non-long shifts, create uncovered shifts as before
