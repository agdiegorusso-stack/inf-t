            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl transform transition-all" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        ...
                        {/* New fields for long shift availability */}
                        <div className="border-t border-gray-200 pt-3">
                            <h5 className="text-sm font-semibold text-gray-700 mb-2">Disponibilità Turni Lunghi</h5>
                            <div className="flex items-center mb-2">
                                <input 
                                    id={`availableForLongShifts-${staff.id}`} 
                                    type="checkbox" 
                                    checked={formData.availableForLongShifts || false} 
                                    onChange={(e) => handleInputChange('availableForLongShifts', e.target.checked)}
                                    className="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded"
                                />
                                <label htmlFor={`availableForLongShifts-${staff.id}`} className="ml-2 text-sm text-gray-700">
                                    Disponibile per turni lunghi
                                </label>
                            </div>
                            {formData.availableForLongShifts && (
                                <div className="ml-6">
                                    <label htmlFor={`maxLongShiftsPerMonth-${staff.id}`} className="block text-sm font-medium text-gray-700 mb-1">
                                        Numero massimo di turni lunghi al mese
                                    </label>
                                    <select
                                        id={`maxLongShiftsPerMonth-${staff.id}`}
                                        value={formData.maxLongShiftsPerMonth || 0}
                                        onChange={(e) => handleInputChange('maxLongShiftsPerMonth', parseInt(e.target.value, 10))}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                    >
